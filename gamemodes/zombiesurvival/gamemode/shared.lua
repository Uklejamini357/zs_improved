...
GM.Version	=	"1.7.4"
GM.ReleaseNotes	= "minor shocking update" -- further release notes can be found down after include's and include_library (with using addToNote function)
...
addToNote("Shocking Zapper Arc now inflicts area damage and is now more blue-ish", COLORID_CYAN)

addToNote("Upgrading deployables does not strip the previous upgrade weapon anymore if you have more than 1 of these.", COLORID_GREEN)

addToNote("Fixed buying Shocking Zapper Arc giving wrong item", COLORID_WHITE)
addToNote("Fixed color bug when looking at mini/demi/boss zombie", COLORID_WHITE)
addToNote("Fixed new turrets not being counted to turret channels", COLORID_WHITE)

addToNote("#FIX (1.7.3)", COLORID_GRAY)
...
function GM:PlayerNoClip(pl, on)
	if pl:IsAdmin() and (on or pl:OldAlive()) then
		if SERVER then
