...
function ENT:Draw()
	...
	if not hidepacks or not shadowman then
		...
		cam.Start3D2D(self:LocalToWorld(Vector(self:OBBMaxs().x, 0, 2.5)), ang, 0.016)
			draw.RoundedBox(64, w * -0.5, h * -0.5, w, h, color_black_alpha120)
			draw.SimpleText(translate.Get("weapon_remantler"), "ZS3D2DFont2", 0, 0, COLOR_GREEN, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		cam.End3D2D()
	end
end
