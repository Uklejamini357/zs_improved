...
function SWEP:Think()
	...
	if self:GetBattlecry() > CurTime() then
		if self.BattlecryInterval < CurTime() then
			...
			if SERVER then
				for _, ent in ipairs(ents.FindInSphere(center, 120)) do
...
function SWEP:SecondaryAttack()
	...
	if SERVER then
		owner:EmitSound("npc/stalker/go_alert2a.wav", 100, math.random(40, 44))
		util.ScreenShake(pos, 5, 5, 3, 560)

		local center = owner:WorldSpaceCenter()
		timer.Simple(0, function() Battlecry(center) end)

		for _, ent in ipairs(ents.FindInSphere(center, 225)) do
